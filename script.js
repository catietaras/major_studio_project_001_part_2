...
function startCard(object) {
  return `<button class="start-card" data-start="${escapeHtml(object.id)}"><span class="start-card-image"><img src="${escapeHtml(object.image)}" alt="${escapeHtml(object.imageAlt || object.title)}"></span><span class="start-card-copy"><span class="start-card-floor">FLOOR ${object.gallery.floor}</span><strong>${escapeHtml(kidTitle(object))}</strong><span class="start-card-action">Pick me! <span aria-hidden="true">→</span></span></span></button>`;
}
function renderStart() {
  state.journey = [];
  const starters = START_IDS.map(id => state.byId.get(id)).filter(object => object && onView(object));
  app.innerHTML = `<section class="shell kid-start"><div class="start-heading"><span class="start-sparkle" aria-hidden="true">✦</span><h1 class="display">What do you see?</h1><p>Pick something you see. Then follow clues to find four more.</p></div><div class="start-cards">${starters.map(startCard).join('')}</div></section>`;
...
function choiceLabel(link) {
  return link.route === 'washington' ? 'Find more George!' : `Follow a clue: ${ideaLabel(link.value)}`;
}
function renderObject() {
  ...
  const choicesHtml = count === MAX_STOPS
    ? `<div class="choices"><h2>You found all five!</h2><p>Your treasure map is ready.</p><button class="button red" id="see-journey">See my treasure map <span aria-hidden="true">→</span></button></div>`
    : `<div class="choices"><h2>Pick a clue!</h2><p>Which object will you find next?</p><div class="choice-grid">${state.choices.map((link,index) => { const target = state.byId.get(link.targetId); return `<button class="choice" data-choice="${index}"><span class="choice-number">${link.route === 'washington' ? '★ GEORGE' : '✦ NEW CLUE'}</span><img class="choice-thumb" src="${escapeHtml(target.image)}" alt=""><strong>${escapeHtml(choiceLabel(link))}</strong><small>${escapeHtml(kidTitle(target))}</small><span class="choice-place">${escapeHtml(placeLabel(target))} →</span></button>`; }).join('')}</div>${state.choices.length ? '' : '<button class="button" id="return-map">Pick a new first object</button>'}</div>`;
...
function renderJourney() {
  ...
  app.innerHTML = `<section class="shell kid-finale"><div class="final-hero"><span class="eyebrow">You did it!</span><h1 class="section-title">Your treasure map</h1><p>Follow the arrows from 1 to 5.</p></div><div class="treasure-map" style="aspect-ratio:1000/${mapHeight}" role="group" aria-label="Your five-stop museum treasure map">${bands}<svg class="treasure-trail" viewBox="0 0 1000 ${mapHeight}" preserveAspectRatio="none" aria-hidden="true"><defs><marker id="trail-arrow" viewBox="0 0 12 12" markerWidth="12" markerHeight="12" refX="11" refY="6" orient="auto" markerUnits="userSpaceOnUse"><path d="M1 1 L11 6 L1 11 Z" fill="#a86446"/></marker></defs>${arrows}</svg>${markers}</div><p class="treasure-note">The map shows your floors and room areas. Ask a museum guide for exact spots.</p><div class="final-actions"><button class="button" id="again">Play again <span aria-hidden="true">↗</span></button></div></section>`;
